...
export const reducerVotes = (state, action) => {
  switch (action.type) {
    case "VOTE QUICK": {
      if (action.id != null) {
        let newState = state.map((vote) => {
          //update the state for the winner team
          if (vote.id === action.id) {
            return { ...vote, quickScore: true, win: !vote.win, score: 1000 };
          } else {
            if (vote.quickScore) {
              return vote;
            } //quickVote already set
            else {
              return { ...vote, quickScore: true, win: false, score: -1 };
            } // reintialized to quickScore setup
          }
        });
        if (newState[0].win && newState[1].win) {
          newState = newState.map((vote) => {
            //update the state for the winner team
            return { ...vote, draw: true };
          });
        } else {
          newState = newState.map((vote) => {
            //check if both team win => it's a draw
      ...
      if (action.id != null) {
        let newState = state.map((vote) => {
          ...
          if (vote.id === action.id) {
            ...
            if (vote.quickScore) {
              //initialize the other one to score with 0
        ...
        if (newState[0].score == newState[1].score) {
          ...
          } else if (newState[0].score < newState[1].score) {
            //team 2 should win
