...
const VoteCards = ({ match, dataVote }) => {
    ...
    if (dataVote.length > 0) {
        const isQuickScore = dataVote[0].score_away == 1000 || dataVote[0].score_home == 1000;
        const isDraw = (dataVote[0].score_away == dataVote[0].score_home) && dataVote[0].score_away != 0 ? true : false;
        const firstTeamWin = !isDraw && (dataVote[0].score_home > dataVote[0].score_away) ? true : false;
    ...
    const [showQuickVote, setShowQuickVote] = useState(initialVotesData[0].quickScore);
    ...
    const ControlButton = (props) => {
        const color = (isSavedError ? "bg-error" : (isSaved ? "bg-valide" : "bg-kasar3"))
        return (
            <div className="text-center flex">
                ...
                <div className="flex-1">
                    < button onClick={saveVoteHandler}
                        className={`text-kasar1 font-bold ${color} border border-kasar2 rounded-lg p-2 hover:bg-kasar3 my-2`}>{isSavedError ? "Error!" : (isSaved ? "Saved" : "Save Vote")}</button>
                </div>
            </div >
        )
    }
    ...
    return (
        <div key={match.match_id} className="p-6 rounded-lg border border-gray-200 shadow-md bg-kasar1">
